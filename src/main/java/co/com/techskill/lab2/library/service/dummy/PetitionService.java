    ...
    public Flux<String> processPetition(PetitionDTO petitionDTO) {
        return dummyFindAll()
                .filter(petition -> petition.getType().equals("RETURN") &&
                        !petition.getSentAt().isBefore(LocalDate.now().minusDays(3))) // "RETURN".equals(petition.getType()))
                .switchIfEmpty(Mono.error(new RuntimeException("Petition of type RETURN not found or > 3 days")))
